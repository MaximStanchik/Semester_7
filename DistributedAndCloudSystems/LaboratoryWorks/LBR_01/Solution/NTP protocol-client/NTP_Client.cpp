...
int main()
{
	...
	while (true) {
		...
		try
		{
			...
			cout << " * Init sync: " << FormatTime(ClientTime) << endl << endl;
			...
			for (int i = 0; i < 30; i++)
			{
				...
				cout << "Client time: " << FormatTime(ClientTime) << " Local time: " << FormatTime(localUnixTime) << " Correction: " << ss.corrTime << endl;
				if (ClientTime > localUnixTime)
				{
					cout << "The local time is " << FormatTime(ClientTime - localUnixTime) << " behind of the global" << endl;
				}
				else
				{
					cout << "The local time is " << FormatTime(localUnixTime - ClientTime) << " ms ahead of the the global" << endl;
				}
				cout << endl;
			}
