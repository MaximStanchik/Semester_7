...
class _EmployeeListScreenState extends State<EmployeeListScreen> {
  ...
  String? _pendingDeletedEmployeeKey;
  ...
  Future<void> _deleteEmployee(BuildContext context, Employee employee) async {
    ...
    if (confirmed == true) {
      _pendingDeletedEmployeeKey = (employee.firestoreId?.isNotEmpty == true)
    ...
    return BlocListener<EmployeeBloc, EmployeeState>(
      listener: (context, state) {
        if (state is EmployeeError) {
          _pendingDeletedEmployeeKey = null;
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Ошибка удаления: ${state.message}')),
          );
          return;
        }

        if (state is EmployeeLoaded && _pendingDeletedEmployeeKey != null) {
          final key = _pendingDeletedEmployeeKey!;
          final stillExists = state.employees.any(
            (e) => (e.firestoreId ?? '').trim() == key || e.email.trim() == key,
          );
          if (!stillExists) {
            _pendingDeletedEmployeeKey = null;
            ScaffoldMessenger.of(context).showSnackBar(
