...
class HiveService {
  ...
  Future<void> init() async {
    ...
    _usersBox = await _openEncryptedBoxWithRecovery<AppUser>(_usersBoxName);
    _productsBox = await _openEncryptedBoxWithRecovery<Product>(_productsBoxName);
    _favoritesBox = await _openEncryptedBoxWithRecovery<FavoriteEntry>(_favoritesBoxName);
    _compressedBox = await _openEncryptedBoxWithRecovery<Uint8List>(_compressedBoxName);
    _reviewsBox = await _openEncryptedBoxWithRecovery<Review>(_reviewsBoxName);
  ...
  Future<Uint8List> _loadOrCreateKey(String fileName, {bool cache = true}) async {
    ...
    if (await file.exists()) {
      ...
      final regenerated = Uint8List.fromList(Hive.generateSecureKey());
      await file.writeAsBytes(regenerated, flush: true);
      if (cache && fileName == 'hive_primary.key') {
        _primaryKey = regenerated;
      }
      return regenerated;
    }
  ...
  Future<Box<T>> _openEncryptedBoxWithRecovery<T>(String name) async {
    try {
