...
public class ResultsService : IResultsService
{
    ...
    public StorageResponseDTO Add(string value)
    {
        ...
        try
        {
            var newItem = new StorageResponseDTO(_nextId, value);
    ...
    public StorageResponseDTO? Delete(int id)
    {
        _fileLock.Wait();
        try
        {
            ...
            _items.Remove(id);
            _ = SaveToFileAsync();
            return new StorageResponseDTO(id, itemToDelete);
        }
        finally
        {
            _fileLock.Release();
    ...
    public StorageResponseDTO? GetById(int id)
    {
        _fileLock.Wait();
        try
        {
            ...
            return new StorageResponseDTO(id, result);
        }
        finally
        {
            _fileLock.Release();
    ...
    public StorageResponseDTO? Update(int id, string value)
    {
        if (string.IsNullOrWhiteSpace(value))
            throw new ArgumentException("Value cannot be empty");
        ...
        try
        {
            ...
            _items[id] = value;
            _ = SaveToFileAsync();
            return new StorageResponseDTO(id, _items[id]);
        }
        finally
        {
            _fileLock.Release();
    ...
    private async Task LoadFromFileAsync()
    {
        ...
        try
        {
            ...
            var data = JsonConvert.DeserializeObject<StorageRequestDTO>(json);
            if (data != null)
            {
                _items = data.getItems() ?? new Dictionary<int, string>();
                _nextId = data.getNextId();
    ...
    private async Task SaveToFileAsync()
    {
        ...
        try
        {
            var data = new StorageRequestDTO(_items, _nextId);
